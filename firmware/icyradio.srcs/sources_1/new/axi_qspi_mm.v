...
#(
    ...
    localparam S_AXI_LITE_ASZ = 5, // ceil(log2(<number of 32-bit registers>)) + 2
...
localparam SPI_MMIO_FSM_STATE_READY = 3'd0;
localparam SPI_MMIO_FSM_STATE_XFER_ADDR = 3'd1;
localparam SPI_MMIO_FSM_STATE_XFER_MODE = 3'd2;
localparam SPI_MMIO_FSM_STATE_XFER_DUMMY = 3'd3;
localparam SPI_MMIO_FSM_STATE_CLEAR_IN_BUF = 3'd4;
localparam SPI_MMIO_FSM_STATE_XFER_DATA = 3'd5;
localparam SPI_MMIO_FSM_STATE_RAISE_CS = 3'd6;
localparam SPI_MMIO_FSM_STATE_WAIT_CS = 3'd7;
...
wire                    spi_io2_t;
wire                    spi_io3_t;
wire              [3:0] spi_io_i = {spi_io3_i, spi_io2_i, spi_io1_i, spi_io0_i};
reg               [3:0] spi_io_o;
reg               [3:0] spi_io_t;
...
reg               [7:0] spi_mmio_cs_size; // Total CS high cycles (1 cycle = 1 aclk cycle)
reg               [7:0] spi_mmio_cs_rem; // Remaining CS high cycles (1 cycle = 1 aclk cycle)
    ...
    begin
        if(!aresetn)
            begin
                ...
                spi_mmio_cs_size <= 8'd0;
                spi_mmio_cs_rem <= 8'd0;
            ...
            begin
                ...
                if(s_axi_lite_rd_en)
                    begin
                        ...
                        case(s_axi_lite_araddr[S_AXI_LITE_ASZ - 1:S_AXI_LITE_ADDR_LSB])
                            3'h0:    s_axi_lite_rdata <= {16'd1, 8'd0, 8'd0}; // IP Version
                            3'h1:    s_axi_lite_rdata <= {1'd0, spi_io_mode_q, 4'd0, spi_fsm_state, spi_dir, spi_idle, 2'd0, spi_sr_in_buf_valid, spi_sr_out_buf_valid, 2'd0, spi_mmio_en, spi_mmio_en_req, 3'd0, spi_lsb_first, 1'd0, spi_io_mode, spi_mode, spi_en, spi_sck_div_en};
                            3'h2:    s_axi_lite_rdata <= {{(S_AXI_LITE_DSZ - SCK_DIV_SZ){1'b0}}, spi_sck_div};
                            3'h3:    s_axi_lite_rdata <= {spi_mmio_cs_rem, spi_mmio_cs_size, spi_mmio_mode_bits, spi_mmio_dummy_rem, spi_mmio_dummy_size, spi_mmio_addr_rem, spi_mmio_addr_size};
                            3'h4:    s_axi_lite_rdata <= {22'd0, spi_rd_req, spi_sr_out_buf_valid, spi_sr_out_buf};
                            3'h5:    {spi_sr_in_buf_ready, s_axi_lite_rdata} <= {spi_sr_in_buf_valid & ~spi_mmio_en, spi_sr_in_buf_valid, 23'd0, spi_sr_in_buf};
                            3'h6:    s_axi_lite_rdata <= {24'd0, spi_sr};
                            3'h7:    s_axi_lite_rdata <= {{(S_AXI_LITE_DSZ - NUM_SLAVES){1'b0}}, spi_ss_o};
                                ...
                                begin
                                    if(!spi_mmio_en)
                                        begin
                                            if(s_axi_lite_wstrb[0]) // s_axi_lite_wdata[7:0]
                                                begin
                                                    spi_mmio_addr_size <= s_axi_lite_wdata[1:0];
                                                    // s_axi_lite_wdata[3:2] is spi_mmio_addr_rem
                                                    spi_mmio_dummy_size <= s_axi_lite_wdata[5:4];
                                                    // s_axi_lite_wdata[7:6] is spi_mmio_dummy_rem
                                                end

                                            if(s_axi_lite_wstrb[1]) // s_axi_lite_wdata[15:8]
                                                begin
                                                    spi_mmio_mode_bits <= s_axi_lite_wdata[15:8];
                                                end

                                            if(s_axi_lite_wstrb[2]) // s_axi_lite_wdata[23:16]
                                                begin
                                                    spi_mmio_cs_size <= s_axi_lite_wdata[23:16];
                                                end

                                            // if(s_axi_lite_wstrb[3]) // s_axi_lite_wdata[31:24]
                                            //     begin
                                            //         // s_axi_lite_wdata[31:24] is spi_mmio_cs_rem
                                            //     end
                                        end
                                end
                            3'h4: // Register 4
                ...
                case(spi_mmio_fsm_state)
                    SPI_MMIO_FSM_STATE_READY:
                        begin
                            s_axi_full_arready <= !s_axi_full_rvalid; // Ready to receive a read address if no read data is pending
                        ...
                        begin
                            if(!spi_sr_out_buf_valid)
                                begin
                                    ...
                                    if(spi_mmio_dummy_rem == 2'd0)
                                        spi_mmio_fsm_state <= SPI_MMIO_FSM_STATE_CLEAR_IN_BUF;
                                    else
                    ...
                    SPI_MMIO_FSM_STATE_CLEAR_IN_BUF:
                        begin
                            spi_sr_in_buf_ready <= 1'b1; // Signal ready to receive the next byte

                            // If no pending transfers, move on
                            if(!spi_rd_req && !spi_sr_out_buf_valid && !spi_sr_in_buf_valid)
                                spi_mmio_fsm_state <= SPI_MMIO_FSM_STATE_XFER_DATA;
                        end
                    SPI_MMIO_FSM_STATE_XFER_DATA:
                        begin
                            // Always request data, no problem if we over-read
                            if(spi_mmio_en && !spi_rd_req && !spi_sr_in_buf_valid)
                                spi_rd_req <= 1'b1;
                        ...
                        begin
                            if(spi_idle)
                                begin
                                    ...
                                    spi_mmio_cs_rem <= spi_mmio_cs_size;
                    ...
                    SPI_MMIO_FSM_STATE_WAIT_CS:
                        begin
                            spi_mmio_cs_rem <= spi_mmio_cs_rem - 1;
