...
uint64_t SI5351_XTAL_FREQ;
uint64_t SI5351_CLKIN_FREQ;
uint64_t SI5351_CLKIN_DIV_FREQ;
uint64_t SI5351_PLL_SRC_FREQ[2];
uint64_t SI5351_PLL_FREQ[2];
uint64_t SI5351_MS_SRC_FREQ[8];
uint64_t SI5351_MS_FREQ[8];
uint64_t SI5351_CLK_SRC_FREQ[8];
uint64_t SI5351_CLK_FREQ[8];

void *pBAR;

void set_pbar(void *p)
{
    pBAR = p;
}
...
static uint8_t si5351_read_register(uint8_t ubRegister)
{
    ...
    // {
        if(!i2c_write_byte(pBAR + 0x00016000, SI5351_I2C_ADDR, ubRegister, 0))
            DBGPRINTLN_CTX("i2c_write_byte failed");

        return i2c_read_byte(pBAR + 0x00016000, SI5351_I2C_ADDR, 1);
    // }
}
static void si5351_write_register(uint8_t ubRegister, uint8_t ubValue)
{
    ...
    // {
        ...
        if(!i2c_write(pBAR + 0x00016000, SI5351_I2C_ADDR, pubBuffer, 2, 1))
            DBGPRINTLN_CTX("i2c_write failed");
    // }
}
...
uint8_t si5351_init()
{
    // if(!i2c0_write(SI5351_I2C_ADDR, NULL, 0, 1)) // Check ACK from the expected address
    //     return 0;
...
void si5351_isr()
{
    ...
    ubFlags = si5351_read_register(SI5351_REG_IRQ_FLAGS);
...
{
    si5351_mixed_number_t *pMultiplier;
    uint32_t ulP1, ulP2, ulP3;
    ...
    pMultiplier = (si5351_mixed_number_t *)kzalloc(sizeof(si5351_mixed_number_t), GFP_KERNEL);
    ...
    ulP1 = 128 * pMultiplier->ulInt + (uint32_t)((128 * pMultiplier->ulNum) / pMultiplier->ulDen) - 512;
    // uint32_t ulP2 = 128 * pMultiplier->ulNum - pMultiplier->ulDen * (uint32_t)(128.f * (double)pMultiplier->ulNum / pMultiplier->ulDen);
    ulP2 = 128 * pMultiplier->ulNum - pMultiplier->ulDen * (uint32_t)((128 * pMultiplier->ulNum) / pMultiplier->ulDen);
    ulP3 = pMultiplier->ulDen;

    DBGPRINTLN_CTX("ulP1: %lu, ulP2: %lu, ulP3: %lu", ulP1, ulP2, ulP3);
...
{
    si5351_mixed_number_t *pDivider;
    uint32_t ulP1, ulP2, ulP3;
    ...
    pDivider = (si5351_mixed_number_t *)kzalloc(sizeof(si5351_mixed_number_t), GFP_KERNEL);
    ...
    else
    {
        ...
        ulP1 = 128 * pDivider->ulInt + (uint32_t)((128 * pDivider->ulNum) / pDivider->ulDen) - 512;
        // uint32_t ulP2 = 128 * pDivider->ulNum - pDivider->ulDen * (uint32_t)(128.f * (double)pDivider->ulNum / pDivider->ulDen);
        ulP2 = 128 * pDivider->ulNum - pDivider->ulDen * (uint32_t)((128 * pDivider->ulNum) / pDivider->ulDen);
        ulP3 = pDivider->ulDen;
