...
*/
    ...
    while(1)
    {
        e = libusb_bulk_transfer(pLibUSBDeviceHandle, ICYRADIO_USB_RX_ENDPOINT, ubBuf, sizeof(ubBuf), &act_len, 0);
        ...
        {
            /*
            fprintf(stderr, "libusb_bulk_transfer transfered %d\r\n", act_len);

            fprintf(stderr, "[");

            for(int i = 0; i < (act_len > 16 ? 16 : act_len); i++)
                fprintf(stderr, "%02X ", ubBuf[i]);

            fprintf(stderr, "]\r\n");
            */

           fwrite(ubBuf, sizeof(ubBuf), 1, stdout);
        }
    }
	...
	e = libusb_control_transfer(pLibUSBDeviceHandle, ICYRADIO_USB_CTRL_OUT_ENDPOINT, 0, 0xAAAA, 0x5555, ubBuf, sizeof(ubBuf), 300);
    ...
    {
		fprintf(stderr, "libusb_control_transfer transfered %d\r\n", e);
