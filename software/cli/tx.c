...
static uint32_t ulSelectedTransfer = 0;
static uint32_t ulSelectedTransferFilled = 0;
...
int main(int argc, char *argv[])
{
	...
	for(uint32_t i = 0; i < LIBUSB_BUF_NUMBER; i++)
    {
		libusb_fill_bulk_transfer(ppLibUSBTransfer[i], pLibUSBDeviceHandle, ICYRADIO_USB_TX_ENDPOINT, ppubLibUSBTransferBuffer[i], LIBUSB_BUF_LENGTH, libusb_callback, NULL, 0);
    ...
    while(iShutdown < 3)
    {
		...
		if(iShutdown == 0)
        {
            struct libusb_transfer *pSelectedLibUSBTransfer = ppLibUSBTransfer[ulSelectedTransfer];
            uint8_t *pubSelectedBuffer = ppubLibUSBTransferBuffer[ulSelectedTransfer];

            if(pSelectedLibUSBTransfer->status == LIBUSB_TRANSFER_COMPLETED)
            {
                ulSelectedTransferFilled += fread(pubSelectedBuffer, 1, LIBUSB_BUF_LENGTH - ulSelectedTransferFilled, stdin);

                if(ulSelectedTransferFilled == LIBUSB_BUF_LENGTH)
                {
                    libusb_submit_transfer(pSelectedLibUSBTransfer);

                    ulSelectedTransferFilled = 0;
                    ulSelectedTransfer++;

                    if(ulSelectedTransfer >= LIBUSB_BUF_NUMBER)
                        ulSelectedTransfer = 0;
                }
            }
        }
		else if(iShutdown == 1)
