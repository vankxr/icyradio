...
uint8_t axi_iic_transmit(uint8_t ubInst, uint8_t ubAddress, uint8_t *pubSrc, uint8_t ubCount, uint8_t ubStop)
{
    ...
    axi_iic_reg_write(ubInst, AXI_IIC_REG_TX_FIFO, AXI_IIC_REG_TX_FIFO_START | ubAddress);
    ...
    {
        while(ulFlags & AXI_IIC_REG_IxR_INT4_BUS_NOT_BUSY)
        {
    ...
    do
    {
        if(ubAddress & 1) // Read
        {
            ...
            while(axi_iic_reg_read(ubInst, AXI_IIC_REG_SR) & AXI_IIC_REG_SR_RX_FIFO_EMPTY)
            {
                ulFlags = axi_iic_reg_read(ubInst, AXI_IIC_REG_ISR);
        ...
        else // Write
        {
            ...
            while(!(axi_iic_reg_read(ubInst, AXI_IIC_REG_SR) & AXI_IIC_REG_SR_TX_FIFO_EMPTY))
            {
                ulFlags = axi_iic_reg_read(ubInst, AXI_IIC_REG_ISR);
