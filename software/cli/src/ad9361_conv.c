 ...
 */
static int32_t ad9361_check_pn(struct ad9361_rf_phy* phy, bool tx, unsigned int delay)
{
    ...
    for(chan = 0; chan < num_chan; chan++)
        axi_adc_write(axi_adc, AXI_ADC_REG_CHAN_STATUS(chan), AXI_ADC_PN_ERR | AXI_ADC_PN_OOS);
    ...
    for(chan = 0; chan < num_chan; chan++)
    {
        axi_adc_read(axi_adc, AXI_ADC_REG_CHAN_STATUS(chan), &adi_reg_chan_status);
...
int32_t ad9361_hdl_loopback(struct ad9361_rf_phy* phy, bool enable)
{
    ...
    if(AXI_CORE_VERSION_MAJOR(version) > 7)
        addr = 0x4418;
    else
        addr = 0x4414;
...
static int32_t ad9361_iodelay_set(struct axiadc_state* st, unsigned lane, unsigned val, bool tx)
{
    ...
    if(tx)
        if(AXI_CORE_VERSION_MAJOR(st->pcore_version) > 8)
...
static int32_t ad9361_dig_tune_iodelay(struct ad9361_rf_phy* phy, bool tx)
{
    ...
    for(uint8_t i = 0; i < 7; i++)
    {
        ...
        DBGPRINTLN_CTX("Lane %"PRId32", window cnt %"PRIu32" , start %"PRIu32", IODELAY set to %"PRIu32, i, c0, s0, s0 + c0 / 2);
    }
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
static void ad9361_dig_tune_verbose_print(struct ad9361_rf_phy* phy, uint8_t field[][16], bool tx, int32_t sel_clk, int32_t sel_data)
{
    char c;

    DBGPRINTLN_CTX("SAMPL CLK: %"PRIu32" tuning: %s ('#' = fail, 'o' = pass, 'O' = selected)", ad9361_clk_get_rate(phy, phy->ref_clk_scale[RX_SAMPL_CLK]), tx ? "TX" : "RX");
    ...
    {
        DBGPRINT_CTX("%"PRIx32"\t", i);

        for(uint8_t j = 0; j < 16; j++)
        {
            if(field[i][j])
                c = '#';
            else if((i == 0 && j == sel_data) || (i == 1 && j == sel_clk))
                c = 'O';
            else
                c = 'o';
 ...
 * @return None.
 */
static void ad9361_set_intf_delay(struct ad9361_rf_phy* phy, bool tx, unsigned int clock_delay, unsigned int data_delay, bool clock_changed)
{
    if(clock_changed)
        ad9361_ensm_force_state(phy, ENSM_STATE_ALERT);

    ad9361_spi_write(REG_RX_CLOCK_DATA_DELAY + (tx ? 1 : 0), RX_DATA_DELAY(data_delay) | DATA_CLK_DELAY(clock_delay));
...
int32_t ad9361_dig_interface_timing_analysis(struct ad9361_rf_phy* phy, char* buf, int32_t buflen)
{
    ...
    ad9361_spi_write(REG_BIST_CONFIG, bist);
    ...
    len += snprintf(buf + len, buflen, "CLK: %"PRIu32" Hz ('#' = fail, 'o' = pass)\n", ad9361_clk_get_rate(phy, phy->ref_clk_scale[RX_SAMPL_CLK]));
    len += snprintf(buf + len, buflen, "D|C-\t");
    ...
    for(uint8_t i = 0; i < 16; i++)
    {
        ...
        for(uint8_t j = 0; j < 16; j++)
            len += snprintf(buf + len, buflen, "%c\t", (field[i][j] ? '#' : 'o'));
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
static int32_t ad9361_dig_tune_delay(struct ad9361_rf_phy* phy, uint32_t max_freq, enum dig_tune_flags flags, bool tx)
{
    ...
    if(((phy->pdata->port_ctrl.pp_conf[2] & LVDS_MODE) || !phy->pdata->rx2tx2))
    ...
    for(uint32_t r = 0; r < (max_freq ? ARRAY_SIZE(rates) : 1); r++)
    {
        if(max_freq)
            ad9361_set_trx_clock_chain_freq(phy, half_data_rate ? rates[r] / 2 : rates[r]);
    ...
    if(!c0 && !c1)
    {
        ...
        DBGPRINTLN_CTX("Tuning %s FAILED!", tx ? "TX" : "RX");
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
static int32_t ad9361_dig_tune_rx(struct ad9361_rf_phy* phy, uint32_t max_freq, enum dig_tune_flags flags)
{
    struct axi_adc* rx_adc = phy->rx_adc;
    int32_t ret;
 ...
 * @return 0 in case of success, negative error code otherwise.
 */
static int32_t ad9361_dig_tune_tx(struct ad9361_rf_phy* phy, uint32_t max_freq, enum dig_tune_flags flags)
{
    struct axiadc_converter* conv = phy->adc_conv;
    struct axi_adc* rx_adc = phy->rx_adc;
    ...
    for(chan = 0; chan < num_chan; chan++)
    {
        ...
        axi_adc_write(rx_adc, AXI_ADC_REG_CHAN_CNTRL(chan), AXI_ADC_FORMAT_SIGNEXT | AXI_ADC_FORMAT_ENABLE | AXI_ADC_ENABLE | AXI_ADC_IQCOR_ENB);
        axi_adc_set_pnsel(phy->rx_adc, chan, AXI_ADC_PN_CUSTOM);
        axi_adc_read(rx_adc, 0x4414 + (chan) * 0x40, &saved_chan_ctrl6[chan]);

        if(AXI_CORE_VERSION_MAJOR(hdl_dac_version) > 7)
        {
    ...
    if(AXI_CORE_VERSION_MAJOR(hdl_dac_version) < 8)
    {
    ...
    for(chan = 0; chan < num_chan; chan++)
    {
        axi_adc_write(rx_adc, AXI_ADC_REG_CHAN_CNTRL(chan), saved_chan_ctrl0[chan]);
        axi_adc_set_pnsel(phy->rx_adc, chan, AXI_ADC_PN9);

        if(AXI_CORE_VERSION_MAJOR(hdl_dac_version) > 7)
        {
            axi_adc_write(rx_adc, 0x4418 + chan * 0x40, saved_dsel[chan]);
            axi_adc_write(rx_adc, 0x4044, 1);
        }
...
int32_t ad9361_dig_tune(struct ad9361_rf_phy* phy, uint32_t max_freq, enum dig_tune_flags flags)
{
    ...
    DBGPRINTLN_CTX("freq: %"PRIu32", flags: 0x%X", max_freq, flags);
    ...
    {
        ad9361_ensm_force_state(phy, ENSM_STATE_ALERT);
        ad9361_spi_write(REG_RX_CLOCK_DATA_DELAY, phy->pdata->port_ctrl.rx_clk_data_delay);
        ad9361_spi_write(REG_TX_CLOCK_DATA_DELAY, phy->pdata->port_ctrl.tx_clk_data_delay);
    }
    else if(!(flags & SKIP_STORE_RESULT))
    {
        phy->pdata->port_ctrl.rx_clk_data_delay = ad9361_spi_read(REG_RX_CLOCK_DATA_DELAY);
        phy->pdata->port_ctrl.tx_clk_data_delay = ad9361_spi_read(REG_TX_CLOCK_DATA_DELAY);
    }
...
int32_t ad9361_post_setup(struct ad9361_rf_phy* phy)
{
    ...
    uint32_t tmp, num_chan, flags, id;
    int32_t ret;
    ...
    if(!rx2tx2)
    {
        ...
        axi_adc_write(rx_adc, 0x404c, (phy->pdata->port_ctrl.pp_conf[2] & LVDS_MODE) ? 1 : 0); /* RATE */
    }
    else
    {
        tmp &= ~BIT(5);
        axi_adc_write(rx_adc, 0x4048, tmp);
        axi_adc_write(rx_adc, 0x404c, (phy->pdata->port_ctrl.pp_conf[2] & LVDS_MODE) ? 3 : 1); /* RATE */
    }

    for(uint32_t i = 0; i < num_chan; i++)
    {
        axi_adc_write(rx_adc, AXI_ADC_REG_CHAN_CNTRL_1(i), AXI_ADC_DCFILT_OFFSET(0));
        axi_adc_write(rx_adc, AXI_ADC_REG_CHAN_CNTRL_2(i), (i & 1) ? 0x00004000 : 0x40000000);
        axi_adc_write(rx_adc, AXI_ADC_REG_CHAN_CNTRL(i), AXI_ADC_FORMAT_SIGNEXT | AXI_ADC_FORMAT_ENABLE | AXI_ADC_ENABLE | AXI_ADC_IQCOR_ENB);
    }
    ...
    ret = ad9361_dig_tune(phy, 1, (id) ? flags | RESTORE_DEFAULT : flags);
    ...
    {
        ret = ad9361_dig_tune(phy, 1, (id) ? flags | RESTORE_DEFAULT | BE_VERBOSE : flags | BE_VERBOSE);

        if(ret < 0)
            return ret;
    }

    ret = ad9361_set_trx_clock_chain(phy, phy->pdata->rx_path_clks, phy->pdata->tx_path_clks);
