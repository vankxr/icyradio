...
int32_t ad9361_init(struct ad9361_rf_phy **ad9361_phy, AD9361_InitParam *init_param)
{
    ...
    phy->clk_refin = (struct ad9361_clk *)calloc(1, sizeof(*phy->clk_refin));
    ...
    phy->pdata->gain_ctrl.use_rx_fir_out_for_dec_pwr_meas = init_param->gc_use_rx_fir_out_for_dec_pwr_meas_enable;
    ...
    phy->pdata->gain_ctrl.f_agc_large_overload_inc_steps = init_param->fagc_large_overload_inc_steps;
    ...
    /* GPO Control */
    phy->pdata->gpo_ctrl.gpo_manual_mode_en = init_param->gpo_manual_mode_enable;
    phy->pdata->gpo_ctrl.gpo_manual_mode_enable_mask = init_param->gpo_manual_mode_enable_mask;
    ...
    DBGPRINTLN_CTX("AXI_AD9361 core version %u.%u.%u successfully initialized", AXI_CORE_VERSION_MAJOR(phy->adc_state->pcore_version), AXI_CORE_VERSION_MINOR(phy->adc_state->pcore_version), AXI_CORE_VERSION_PATCH(phy->adc_state->pcore_version));
...
int32_t ad9361_get_rx_rf_gain(struct ad9361_rf_phy *phy, uint8_t ch, int32_t *gain_db)
{
    ...
    ret = ad9361_get_rx_gain(phy, ad9361_1rx1tx_channel_map(phy, false, ch + 1), &rx_gain);
...
int32_t ad9361_set_rx_rf_bandwidth(struct ad9361_rf_phy *phy, uint32_t bandwidth_hz)
{
    ...
    if(phy->current_rx_bw_Hz != bandwidth_hz)
        ret = ad9361_update_rf_bandwidth(phy, bandwidth_hz, phy->current_tx_bw_Hz);
    else
        ret = 0;
...
int32_t ad9361_set_rx_sampling_freq(struct ad9361_rf_phy *phy, uint32_t sampling_freq_hz)
{
    ...
    ret = ad9361_calculate_rf_clock_chain(phy, sampling_freq_hz, phy->rate_governor, rx, tx);
    ...
    ret = ad9361_update_rf_bandwidth(phy, phy->current_rx_bw_Hz, phy->current_tx_bw_Hz);
...
{
    *sampling_freq_hz = (uint32_t)ad9361_clk_get_rate(phy, phy->ref_clk_scale[RX_SAMPL_CLK]);
...
int32_t ad9361_set_rx_lo_freq(struct ad9361_rf_phy *phy, uint64_t lo_freq_hz)
{
    ...
    ret = ad9361_clk_set_rate(phy, phy->ref_clk_scale[RX_RFPLL], ad9361_to_clk(lo_freq_hz));
...
{
    *lo_freq_hz = ad9361_from_clk(ad9361_clk_get_rate(phy, phy->ref_clk_scale[RX_RFPLL]));
...
int32_t ad9361_set_rx_fir_config(struct ad9361_rf_phy *phy, AD9361_RXFIRConfig fir_cfg)
{
    ...
    ret = ad9361_load_fir_filter_coef(phy, (enum fir_dest)(fir_cfg.rx | FIR_IS_RX), fir_cfg.rx_gain, fir_cfg.rx_coef_size, fir_cfg.rx_coef);
...
int32_t ad9361_set_rx_rfdc_track_en_dis(struct ad9361_rf_phy *phy, uint8_t en_dis)
{
    ...
    ret = ad9361_tracking_control(phy, phy->bbdc_track_en, phy->rfdc_track_en, phy->quad_track_en);
...
int32_t ad9361_set_tx_attenuation(struct ad9361_rf_phy *phy, uint8_t ch, uint32_t attenuation_mdb)
{
    ...
    ret = ad9361_set_tx_atten(phy, attenuation_mdb, channel == 0, channel == 1, !phy->pdata->update_tx_gain_via_alert);
...
int32_t ad9361_get_tx_attenuation(struct ad9361_rf_phy *phy, uint8_t ch, uint32_t *attenuation_mdb)
{
    ...
    ret = ad9361_get_tx_atten(phy, ad9361_1rx1tx_channel_map(phy, true, ch + 1));
...
int32_t ad9361_set_tx_rf_bandwidth(struct ad9361_rf_phy *phy, uint32_t  bandwidth_hz)
{
    ...
    if(phy->current_tx_bw_Hz != bandwidth_hz)
        ret = ad9361_update_rf_bandwidth(phy, phy->current_rx_bw_Hz, bandwidth_hz);
    else
        ret = 0;
...
{
    *sampling_freq_hz = (uint32_t)ad9361_clk_get_rate(phy, phy->ref_clk_scale[TX_SAMPL_CLK]);
...
int32_t ad9361_set_tx_lo_freq(struct ad9361_rf_phy *phy, uint64_t lo_freq_hz)
{
    ...
    ret = ad9361_clk_set_rate(phy, phy->ref_clk_scale[TX_RFPLL], ad9361_to_clk(lo_freq_hz));
...
{
    *lo_freq_hz = ad9361_from_clk(ad9361_clk_get_rate(phy, phy->ref_clk_scale[TX_RFPLL]));
...
int32_t ad9361_set_tx_fir_config(struct ad9361_rf_phy *phy, AD9361_TXFIRConfig fir_cfg)
{
    ...
    ret = ad9361_load_fir_filter_coef(phy, (enum fir_dest)fir_cfg.tx, fir_cfg.tx_gain, fir_cfg.tx_coef_size, fir_cfg.tx_coef);
...
int32_t ad9361_set_tx_rf_port_output(struct ad9361_rf_phy *phy, uint32_t mode)
{
    ...
    ret = ad9361_rf_port_setup(phy, true, phy->pdata->rf_rx_input_sel, phy->pdata->rf_tx_output_sel);
...
int32_t ad9361_set_trx_fir_en_dis(struct ad9361_rf_phy *phy, uint8_t en_dis)
{
    ...
    if((phy->bypass_rx_fir == phy->bypass_tx_fir) && (phy->bypass_rx_fir == !en_dis))
        return ret;
...
int32_t ad9361_do_dcxo_tune_coarse(struct ad9361_rf_phy *phy, uint32_t coarse)
{
    ...
    return ad9361_set_dcxo_tune(phy, phy->pdata->dcxo_coarse, phy->pdata->dcxo_fine);
}
